...
Trains the same five model types (Linear Regression, Random Forest, XGBoost,
LSTM, GRU) once per horizon (1/5/10/20/60 trading days) per target, inside
the unchanged unified architecture (one model per horizon per type across
all 25 stocks, Ticker as a feature) - but now against Target_Open/
Target_Close instead of Target_High/Target_Low, following Phases/phase_9.md.

Imports from model_utils.py instead of train_models_phase6.py, since Phase 4
and Phase 6 are deleted outright in this phase (see Phases/phase_9.md "Why
this needs a shared module first"). The horizon-aware wrappers below
(evaluate_predictions_h, evaluate_per_stock_h, create_sequences_h,
build_error_distributions_h) are project-specific glue, not generic reusable
code, so they're defined here rather than in model_utils.py.

"Validation set" mapping and the test/validate split discipline are
unchanged from Phase 6/7: everything here is computed on data/test.csv;
data/validate.csv is not touched. See Phases/phase_9_notes.md for the
decisions made in this file.
"""
...
def main():
    ...
    with open(MODELS_DIR / "phase9_feature_scaler.pkl", "wb") as f:
    ...
    for horizon in HORIZONS:
        ...
        for target in TARGETS:
            ...
            with open(MODELS_DIR / f"phase9_lr_{target.lower()}_{horizon}d.pkl", "wb") as f:
                pickle.dump(lr, f)
            ...
            with open(MODELS_DIR / f"phase9_rf_{target.lower()}_{horizon}d.pkl", "wb") as f:
                pickle.dump(rf, f)
            ...
            xgb_model.save_model(str(MODELS_DIR / f"phase9_xgb_{target.lower()}_{horizon}d.json"))
            ...
            tensorboard_cb = TensorBoard(log_dir=str(RUNS_DIR / f"phase9_lstm_{target.lower()}_{horizon}d"))
            ...
            lstm_model.save(str(MODELS_DIR / f"phase9_lstm_{target.lower()}_{horizon}d.keras"))
            ...
            gru_model, pred_gru_ret, gru_loss_history = train_gru(
                ...
                log_dir=RUNS_DIR / f"phase9_gru_{target.lower()}_{horizon}d",
            )
            pred_gru_price = to_price(close_seq, pred_gru_ret)
            torch.save(gru_model.state_dict(), str(MODELS_DIR / f"phase9_gru_{target.lower()}_{horizon}d.pt"))
    ...
    loss_history_df.to_csv(DATA_DIR / "phase9_training_loss_history.csv", index=False)
    print(f"\nSaved epoch-by-epoch train/val loss history ({len(loss_history_df)} rows) to phase9_training_loss_history.csv.")

    overall_df = pd.DataFrame(overall_results).sort_values(["Horizon", "Target", "MAPE"])
    overall_df.to_csv(DATA_DIR / "phase9_model_comparison_overall.csv", index=False)
    ...
    per_stock_df.to_csv(DATA_DIR / "phase9_model_comparison_per_stock.csv", index=False)

    spread_df = pd.DataFrame(spread_rows)
    spread_df.to_csv(DATA_DIR / "phase9_spread_accuracy.csv", index=False)
    ...
    with open(MODELS_DIR / "phase9_error_distributions.pkl", "wb") as f:
    ...
    with open(MODELS_DIR / "phase9_ensemble_weights.pkl", "wb") as f:
    ...
    width_df.to_csv(DATA_DIR / "phase9_range_width_by_horizon.csv", index=False)
